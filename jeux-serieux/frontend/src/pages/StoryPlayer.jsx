...
function Confetti() {
  const pieces = Array.from({ length: 20 }, (_, i) => ({
    ...
    delay: Math.random() * 1, size: 8 + Math.random() * 8,
  }));
  return (
...
export default function StoryPlayer() {
  ...
  // phase : 'listen' | 'quiz' | 'score' | 'feedback' | 'alreadyDone'
  ...
  const handleListen = () => {
    if (!('speechSynthesis' in window)) { alert('Ton navigateur ne supporte pas la lecture vocale.'); return; }
  ...
  const revealHint = (qi) => setRevealedHints((prev) => ({ ...prev, [qi]: true }));
  ...
  const submitAnswers = async () => {
    ...
    try {
      ...
      if (err.response?.status === 400 && msg.toLowerCase().includes('termin')) setPhase('alreadyDone');
      else setError(msg || 'Erreur lors de la soumission');
  ...
  const callFinish = async () => {
    setSubmitting(true);
    try {
      const res = await api.post(`/progress/${assignmentId}/finish`);
      setFinalResult(res.data);
      setPhase('feedback');
      if (res.data.stars >= 2) { setShowConfetti(true); setTimeout(() => setShowConfetti(false), 3500); }
    } catch (err) {
  ...
  const retryFromListen = () => {
    setAnswers({}); setRevealedHints({}); setCurrentQuestion(0);
    setListenCompleted(false); setScoreResult(null); setIsSecondAttempt(true);
    setPhase('listen');
  };
  ...
  return (
    <div className="min-h-screen teacher-bg p-4">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {user?.role === 'student' && phase !== 'alreadyDone' && (
          <div className="flex gap-2 mb-6 justify-center">
            {[{ id: 'listen', label: '🎧 Écoute' }, { id: 'quiz', label: '🧠 Défis' }].map((p) => {
              const active = p.id === 'listen' ? phase === 'listen' : ['quiz', 'score', 'feedback'].includes(phase);
              return (
        ...
        {phase === 'listen' && (
          <motion.div className="card" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            <div className="text-center mb-6">
              ...
              <h1 className="text-3xl font-fredoka font-bold text-[var(--text-strong)] mb-1">{assignment.story.title}</h1>
              <div className="flex items-center justify-center gap-3 mt-2 text-sm font-bold text-[var(--text-muted)]">
                <span>Niveau {assignment.level}</span><span>·</span><span>{OBJECTIVE_LABEL[assignment.objective]}</span>
              </div>
            </div>

            {/* Texte : visible UNIQUEMENT pour le prof (l'élève écoute sans lire) */}
            {user?.role === 'teacher' && (
            ...
            {user?.role === 'student' && (
              <div className="rounded-2xl p-6 mb-6 text-center" style={{ background: 'rgba(18,14,36,0.55)', border: '1px solid rgba(150,132,206,0.22)' }}>
                <div className="text-5xl mb-2">🎧</div>
                <p className="text-[var(--text-soft)] font-semibold">Ferme les yeux et écoute bien l'histoire, puis relève les défis !</p>
              </div>
            )}
          ...
          <motion.div className="card" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }}>
            <div className="mb-6">
              <div className="flex justify-between text-sm font-bold text-[var(--text-muted)] mb-2">
                <span>Défi {currentQuestion + 1} / {questions.length}{isSecondAttempt ? '  ·  2ᵉ tentative' : ''}</span>
              ...
              <div className="w-full h-3 rounded-full" style={{ background: 'rgba(18,14,36,0.55)' }}>
                <div className="h-full rounded-full transition-all" style={{ width: `${((currentQuestion + 1) / questions.length) * 100}%`, background: 'linear-gradient(90deg,#8E80F2,#C45EE0)' }} />
              </div>
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={currentQuestion} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                ...
                <h2 className="text-2xl font-fredoka font-black text-[var(--text-strong)] mb-8 text-center leading-tight">{currentQ.question}</h2>

                <div className="space-y-3">
                  {currentQ.type === 'true_false' && [true, false].map((val) => (
                    <button key={String(val)} onClick={() => handleAnswer(val)} className="w-full py-4 rounded-2xl font-fredoka font-black text-xl transition-all"
                  ...
                  {['mcq', 'fill_blank', 'odd_one_out'].includes(currentQ.type) && currentQ.options?.map((opt, i) => (
                    <motion.button key={i} onClick={() => handleAnswer(i)} whileTap={{ scale: 0.97 }} className="w-full flex items-center gap-4 p-4 rounded-2xl font-bold text-left transition-all"
                      ...
                      <span className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-black text-sm"
                        style={answers[currentQuestion] === i ? { background: '#8E80F2', color: 'white' } : { background: 'rgba(150,132,206,0.2)', color: '#9D8CF6' }}>
                        {['A', 'B', 'C', 'D'][i]}
                      </span>
                  ...
                  {currentQ.type === 'order_events' && currentQ.events?.map((evt, i) => {
                    ...
                    return (
                      <motion.button key={i} onClick={() => handleOrderClick(i)} whileTap={{ scale: 0.97 }} className="w-full flex items-center gap-4 p-4 rounded-2xl font-bold text-left transition-all"
                        style={selected ? { border: '3px solid #C45EE0', background: 'rgba(196,94,224,0.14)', color: '#EDE7FB' } : { border: '2px solid rgba(150,132,206,0.28)', background: 'rgba(18,14,36,0.45)', color: '#C5BBE4' }}>
                        <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 font-black text-xl"
                          style={selected ? { background: '#C45EE0', color: 'white', boxShadow: '0 0 14px rgba(196,94,224,0.5)' } : { border: '2px dashed rgba(150,132,206,0.4)', color: 'rgba(150,132,206,0.4)' }}>
                          {selected ? pos + 1 : '?'}
                        </div>
                        <span>{evt}</span>
                      </motion.button>
                  ...
                  {currentQ.type === 'order_events' && (
                    <p className="text-center text-sm font-bold animate-pulse" style={{ color: '#C45EE0' }}>Touche les cases dans l'ordre de l'histoire !</p>
                  )}
                </div>
                ...
                {!isSecondAttempt && currentQ.hint && (
                  <div className="mt-5">
                    {revealedHints[currentQuestion] ? (
                      ...
                      <button onClick={() => revealHint(currentQuestion)} className="w-full py-3 rounded-2xl font-bold transition-all"
                        style={{ background: 'rgba(250,204,21,0.10)', border: '2px solid rgba(250,204,21,0.35)', color: '#fde68a' }}>
                        💡 Voir un indice (½ des points)
                      </button>
                    )}
                  </div>
                )}
            ...
            <div className="mt-8">
              {currentQuestion < questions.length - 1 ? (
                <motion.button onClick={() => setCurrentQuestion((q) => q + 1)} disabled={!isAnswerComplete()} whileTap={{ scale: 0.95 }}
                  ...
                  style={!isAnswerComplete() ? { background: 'rgba(150,132,206,0.15)', color: '#6B6490', cursor: 'not-allowed' } : { background: 'linear-gradient(135deg,#8E80F2,#5E4FDD)', boxShadow: '0 10px 26px rgba(94,79,221,0.45)' }}>
                  Défi Suivant ➡️
                </motion.button>
              ) : (
                <motion.button onClick={submitAnswers} disabled={!isAnswerComplete() || submitting} whileTap={{ scale: 0.95 }}
                  className="w-full py-4 rounded-2xl font-fredoka font-black text-xl text-white transition-all"
                  style={(!isAnswerComplete() || submitting) ? { background: 'rgba(150,132,206,0.15)', color: '#6B6490', cursor: 'not-allowed' } : { background: 'linear-gradient(135deg,#22c55e,#16a34a)', boxShadow: '0 10px 26px rgba(34,197,94,0.4)' }}>
                  {submitting ? '⏳ Calcul du score…' : '🏁 Voir mon score'}
                </motion.button>
              )}
            </div>
          </motion.div>
        ...
        {phase === 'score' && scoreResult && (
          <motion.div className="card text-center" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            <h2 className="text-3xl font-fredoka font-black text-[var(--text-strong)] mb-4">Tentative {scoreResult.attemptNumber} terminée !</h2>
            <div className="rounded-2xl p-5 mb-6" style={{ background: 'rgba(142,128,242,0.14)', border: '1px solid rgba(157,140,246,0.35)' }}>
              <p className="text-[var(--text-soft)] font-semibold mb-1">Ton score</p>
              <p className="font-fredoka font-black text-[var(--text-strong)] text-4xl">{scoreResult.score} / {scoreResult.total}</p>
            </div>

            {scoreResult.canRetry ? (
              <>
                <p className="text-[var(--text-soft)] font-semibold mb-5">
                  Tu peux retenter l'aventure pour t'améliorer — mais en 2ᵉ tentative, chaque bonne réponse compte pour la moitié des points. Sinon, termine pour voir tes corrections.
                </p>
                <motion.button onClick={retryFromListen} whileTap={{ scale: 0.95 }} className="w-full btn-primary text-xl py-4 mb-3">🔄 Refaire l'aventure</motion.button>
                <motion.button onClick={callFinish} disabled={submitting} whileTap={{ scale: 0.95 }} className="w-full font-fredoka font-bold text-lg py-3 rounded-2xl transition-all"
                  style={{ background: 'rgba(52,211,153,0.14)', border: '2px solid rgba(52,211,153,0.45)', color: '#86efac' }}>
                  {submitting ? '⏳ …' : '✅ Finir et voir mes corrections'}
                </motion.button>
              </>
            ) : (
              <>
                <p className="text-[var(--text-soft)] font-semibold mb-5">C'était ta dernière tentative.</p>
                <motion.button onClick={callFinish} disabled={submitting} whileTap={{ scale: 0.95 }} className="w-full btn-primary text-xl py-4">
                  {submitting ? '⏳ …' : '✅ Voir mes corrections'}
                </motion.button>
              </>
            )}
          </motion.div>
        ...
        {/* ═══════════════ FEEDBACK FINAL ═══════════════ */}
        {phase === 'feedback' && finalResult && (
          <motion.div className="card" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
            <div className="text-center mb-6">
              <div className="text-7xl mb-2">{finalResult.stars === 3 ? '🏆' : finalResult.stars >= 1 ? '🎉' : '💪'}</div>
              ...
              <div className="text-5xl mb-3">
                {Array.from({ length: 3 }, (_, i) => (<span key={i} style={{ opacity: i < finalResult.stars ? 1 : 0.25 }}>⭐</span>))}
              </div>
              <p className="font-fredoka font-bold text-[var(--text-soft)] text-xl mb-1">{finalResult.score} / {finalResult.total} bonnes réponses</p>
              <p className="font-black text-[#9D8CF6] text-lg">+{finalResult.pointsEarned} pts</p>

              {finalResult.levelUp && (
                <motion.div className="mt-4 rounded-2xl p-4" style={{ background: 'rgba(250,204,21,0.14)', border: '2px solid rgba(250,204,21,0.5)' }}
                  initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.3 }}>
                  <p className="font-fredoka font-black text-yellow-300 text-xl">🎊 Niveau {finalResult.newLevel} débloqué !</p>
                </motion.div>
              )}
            </div>
              ...
              {finalResult.corrections.map((c) => {
                ...
                return (
                  <div key={c.questionIndex} className="rounded-2xl p-4"
                    style={c.isCorrect ? { background: 'rgba(52,211,153,0.10)', border: '2px solid rgba(52,211,153,0.35)' } : { background: 'rgba(239,68,68,0.10)', border: '2px solid rgba(239,68,68,0.35)' }}>
                    <p className="font-bold text-[var(--text-strong)] mb-2 text-sm">{c.isCorrect ? '✅' : '❌'} {q?.question}</p>
                    {!c.isCorrect && (
                    ...
                    {c.explanation && <p className="text-xs text-[var(--text-muted)] mt-1 italic">💡 {c.explanation}</p>}
                  </div>
                );
              })}
            </div>

            <Link to="/student" className="w-full btn-primary text-lg py-3 block text-center">🏠 Retour à mes aventures</Link>
          </motion.div>
        )}
        ...
        {phase === 'alreadyDone' && (
          <motion.div className="card text-center" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
            <div className="text-7xl mb-3">🏅</div>
            <h2 className="text-3xl font-fredoka font-black text-[var(--text-strong)] mb-3">Aventure terminée !</h2>
            <p className="text-[var(--text-soft)] font-semibold mb-6">Tu as déjà terminé cette aventure. Consulte ton tableau de bord pour voir tes résultats !</p>
            <Link to="/student" className="btn-primary text-lg py-3 px-8 inline-flex">🏠 Retour</Link>
          </motion.div>
        )}
      </div>
    </div>
